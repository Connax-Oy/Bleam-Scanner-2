...
static bool config_service_server_initialized = false;      /**< Flag denoting whether configuration service was initialized or not. */
...
void config_s_server_on_ble_evt(ble_evt_t const *p_ble_evt, void *p_context) {
    if (!config_service_server_initialized || (NULL == p_context) || (NULL == p_ble_evt) || CONFIG_S_STATUS_DONE == config_s_get_status()) {
        return;
    }
 ...
 * @param[in,out]   char_id                  Configuration Service characterisctic ID as defined in @ref config_s_char_t.
 ...
 * @param[in]       char_prop_max_len        Maximum length of the data accepted by the characteristic.
 *
 * @retval NRF_SUCCESS on successful characteristic addition.
 * @retval NRF_ERROR_NULL if any of the provided pointers are NULL.
 * @returns otherwise, an error code from @link_sd_ble_gatts_characteristic_add.
 */
...
uint32_t config_s_server_init(config_s_server_t *p_config_s_server, config_s_server_init_t *p_config_s_server_init) {
    ...
    err_code = err_code | config_s_char_add(p_config_s_server, p_config_s_server_init, CONFIG_S_APPKEY,  0, 1, 0, 16);
    ...
    if(NRF_SUCCESS == err_code) {
        ...
        config_service_server_initialized = true;
    }
...
uint32_t config_s_publish_version(config_s_server_t *p_config_s_server) {
    __LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "Publish version data on handle 0x%04X\r\n", CONFIG_S_UUID | CONFIG_S_VERSION);
...
uint32_t config_s_status_update(config_s_server_t *p_config_s_server, config_s_status_t p_status) {
    __LOG(LOG_SRC_APP, LOG_LEVEL_INFO, "Update status on handle 0x%04X\r\n", CONFIG_S_UUID | CONFIG_S_STATUS);
    if (p_config_s_server == NULL)
        return NRF_ERROR_NULL;
    if (p_config_s_server->conn_handle == BLE_CONN_HANDLE_INVALID) {
        __LOG(LOG_SRC_APP, LOG_LEVEL_WARN, "Connection handle invalid.\r\n");
    ...
    if (NRF_SUCCESS != err_code) {
        __LOG(LOG_SRC_APP, LOG_LEVEL_DBG2, "sd_ble_gatts_hvx result: %04X\r\n", err_code);
